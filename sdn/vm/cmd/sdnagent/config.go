...
func (a *agent) getIntendedNetwork(network api.Network) dg.Graph {
	...
	var (
		vethInIPv4, vethOutIPv4 *net.IPNet
		vethInIPv6, vethOutIPv6 *net.IPNet
		vethInIPs, vethOutIPs   []*net.IPNet
	)
	if ipv4Subnet != nil {
		vethInIPv4, vethOutIPv4 = a.genVethIPsForNetwork(network.LogicalLabel, false)
		vethInIPs = append(vethInIPs, vethInIPv4)
		vethOutIPs = append(vethOutIPs, vethOutIPv4)
	}
	if ipv6Subnet != nil {
		vethInIPv6, vethOutIPv6 = a.genVethIPsForNetwork(network.LogicalLabel, true)
		vethInIPs = append(vethInIPs, vethInIPv6)
		vethOutIPs = append(vethOutIPs, vethOutIPv6)
	}
	var (
...
func (a *agent) getIntendedFwRule(
	...
	if rule.DstSubnet != "" {
		...
		if subnet.IP.To4() != nil {
			ipv4RuleArgs = append(ipv4RuleArgs, "-d", rule.SrcSubnet)
			ipv6 = false
		} else {
			ipv6RuleArgs = append(ipv6RuleArgs, "-d", rule.SrcSubnet)
			ipv4 = false
		}
	}
...
func (a *agent) getEndpointAllIPs(ep api.Endpoint) []net.IP {
	var epIPs []net.IP
	epIPv4, _ := a.getEndpointIP(ep, false)
	if epIPv4 != nil {
